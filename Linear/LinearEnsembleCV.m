...
[N,p]=size(X);
cvparameters = struct('radius',[],'alpha',[],'nlearn',[]);
...
Yhat = zeros(N,nlearn(end),Nalpha);
...
for Qifold = 1:length(Qfolds)
    ...
    for is=1:Nlearn
        s = nlearn(is);
        for j=1:Nalpha
            QFX = Yhat(Qji,1:s,j); QFXJ = Yhat(QJ,1:s,j);
            switch options.mix
                case 'elasticnet'
            ...
            CVError(QJ,j,is) = (QYhat - QYJ).^2;
        end
    end
end

meanCVError = permute(mean(CVError,1),[2 3 1]);
[~,I] = min(meanCVError(:));
[ialph, is] = ind2sub(size(meanCVError),I);
...
if path==1 % return the entire path of nlearn
    ...
    for is=1:Nlearn
        switch options.mix
            case 'elasticnet'
                [~,beta{is},my(is)] = ...
                    elasticnetcv(Y,Yhat(:,1:nlearn(is),ialph),[],'gaussian',optionsEN);
            case 'nonnegative'
                [~,beta{is},my(is)] = nonnegativecv(Y,Yhat(:,1:nlearn(is),ialph),[]);
        end
        cvparameters.nlearn(is) = nlearn(is); % We can't sparsify if path==1
        YhatTrain(:,is) = PredictEnsemble(Yhat(:,1:nlearn(is),ialph),beta{is},my(is));
    end
    ...
    if nlearn(is)>1
        switch options.mix
            case 'elasticnet'
                [~,beta,my] = elasticnetcv(Y,Yhat(:,1:nlearn(is),ialph),[],'gaussian',optionsEN);
            case 'nonnegative'
                [~,beta,my] = nonnegativecv(Y,Yhat(:,1:nlearn(is),ialph),[]);
        end
    else
        beta = 1; my = 0;
    end
    ...
    beta = beta(ind);
    YhatTrain = PredictEnsemble(Yhat(:,ind,ialph),beta,my); 
